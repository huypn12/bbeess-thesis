...
\begin{definition}[Bottom Strongly Connected Component]
    Let $\mathcal{M}=(S,\mathbf{P}, s_{init}, AP,L)$ a DTMC and $S'\in S$ a Strongly Connected
...
\begin{theorem}[Long-run theorem]
    Let $\mathcal{M}=(S,\mathbf{P}, s_{init}, AP,L)$ a DTMC.
    \begin{align*}
...
\begin{definition}[Steady-state distribution]
    Let $\mathcal{M}=(S,\mathbf{P}, s_{init}, AP,L)$ a DTMC and vector $v_t$ be a transient state distribution
    \begin{align*}
...
\begin{definition}[PCTL] The syntax of PCTL is defined as follow
    \begin{align*}
