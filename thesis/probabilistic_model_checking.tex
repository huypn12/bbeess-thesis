...
\begin{example}[Knuth-Yao die]
      ...
      fair coin. The algorithm terminates returning one among six possible outcome of a die tossing
      (from one to six). We formalize the algorithm by a DTMC with 6 BSCCs, each of them represents
      a possible outcome of a die tossing. The probabilities to reach each BSCC among the six is $1/6$.
      \begin{figure}[H]
            \centering
            \begin{forest}
                  for tree={
...
\begin{definition}[Exit rate \cite{katoen2001faster}]
      ...
      Let $\mathcal{C} = (S,\mathbf{R}, \iota_{init}, AP, L)$ be a CTMC. We define the \textit{exit
            rate} $E(s), s\in S$ as the row sum of the rate matrix $\mathbf{R}$
      \begin{align*}
            E(s) = \sum_{s'\in S} \mathbf{R}(s, s')
      \end{align*}
\end{definition}
CTMC is a strong formalism to model systems with regarding to real-time properties. However, PCTL is
not applicable to specify a property in CTMC. As the frameworks constructed in this thesis apply to
DTMC and PCTL, we transform CTMCs into DTMCs through a process of  \textit{uniformization}
\cite{katoen2001faster}. Uniformization discretizes a CTMC by transforming it in to a CTMC which has
uniform exit rates among states.
...
\begin{definition}[CTMC uniformization \cite{katoen2001faster}]
      ...
      \begin{align*}
            \forall s\in S: r \geq E(s), r\in\mathbb{R}_{>0}
      \end{align*}
      ...
      \begin{align*}
            ...
            \begin{cases}
                  ...
                  \frac{\mathbf{R}(s,s')}{r} + 1 - \frac{E(s)}{r} & \text{if $s = s'$}
            \end{cases}
      \end{align*}
\end{definition}
...
unaffected as we transform a CTMC to its equivalent uniformized form.
...
\begin{align*}
      ...
      \llap{$\Leftrightarrow$ \qquad \qquad \qquad} N & \geq 4\frac{\log{\frac{2}{\alpha}}}{\epsilon^2}
\end{align*}
...
\end{align*}
As a new simulation trace $t$ is drawn from $\mathcal{D}$, SPRT updates the set of satisfying traces $A$. It then
computes the likelihood ratio based on $A$:
\begin{align*}
      R = \frac{P(A | p + \delta)}{P(A | p - \delta)}
\end{align*}
...
\begin{example}{Parametric Knuth's die}
      Using the example of parametric Knuth-Yao die $\mathcal{M}_{p}$.
      \begin{lstlisting}
